 ...
 * Every unsafe URL is normalized through the global proxy interceptor.
 */
export const resolveSource = async (url: string): Promise<ResolvedSource> => {
  ...
  if (type === "txt") {
    ...
    const finalSource = resolved && resolved !== url ? resolved : url;
    const proxied = toProxyStreamUrl(finalSource);

    if (proxied !== finalSource) {
  ...
  const proxied = toProxyStreamUrl(url);
  if (proxied !== url) {
    log("SourceResolver: proxying:", proxied);
  }

  return {
    type: type === "unknown" ? "hls" : type,
    resolvedUrl: proxied,
    originalUrl: url,
  };
};